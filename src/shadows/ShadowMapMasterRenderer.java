...
public class ShadowMapMasterRenderer{
	...
	public ShadowMapMasterRenderer(Camera camera) {
		shader = new ShaderProgram("src/shaders/shadowVert","src/shaders/shadowFrag",new int[]{0,1,4},new String[]{"in_position","tex", "cube"});
		shadowBox = new ShadowBox(lightViewMatrix, camera);
		fbo = new Fbo(SHADOW_MAP_SIZE,SHADOW_MAP_SIZE,false);
		fbo.createDepthTextureAttahcment();
	 ...
	 */
	public void render(Map<Integer, List<PointEntity>> entities, Map<Integer, List<PolyEntity>> envs, Light sun) {
		shadowBox.update();
		Vector3f sunPosition = sun.position;
		Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
		prepare(lightDirection, shadowBox);
		entityRenderer.renderEntities(entities);
		entityRenderer.renderEnvironments(envs);
		finish();
	}
